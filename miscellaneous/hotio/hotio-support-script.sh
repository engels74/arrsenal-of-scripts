...
main() {
  ...
  log "Step 3/3: Describe the problem (interactive)"
  local q_summary q_expected q_actual q_started q_freq q_repro q_attempts q_error q_extra image_tag
  q_summary="$(input_single "Provide a short one-line summary of the problem" "" 10)"
  q_expected="$(multiline_input "What did you expect to happen? Provide details." 30)"
  q_actual="$(multiline_input "What actually happened? Provide details and exact behavior." 30)"
  local started_sel
  ...
  q_repro="$(multiline_input "Steps to reproduce (one per line). If not obvious, be explicit." 10)"
  q_attempts="$(multiline_input "What have you already tried to fix it?" 0)"
  q_error="$(multiline_input "Paste the most relevant error lines or stack traces (optional)." 0)"
  q_extra="$(multiline_input "Any additional context (e.g., network setup, mounts, proxies, special hardware). Optional." 0)"
  ...
  echo
  echo "Problem:"
  echo " - Expected:"; echo "$q_expected"; echo
  echo " - Actual:"; echo "$q_actual"; echo
  echo " - Started: $q_started"
  echo " - Frequency: $q_freq"; echo
  echo "Steps to Reproduce:"; echo "$q_repro"; echo
  ...
  echo "Additional Context:"; if [[ -n "$q_extra" ]]; then echo "$q_extra"; else echo "N/A"; fi; echo
  echo "---------------- 8< ----------------"

  # Clipboard (optional)
  if have pbcopy; then
    { echo "[${container}] ${q_summary}"; echo; echo "Environment:"; echo " - Image tag: ${image_tag:-unknown}"; echo " - OS/Arch: ${OS}/${ARCH_RAW}"; echo; echo "Links:"; echo " - Docker Logs: ${logs_url:-"(not uploaded)"}"; echo " - Docker Compose: ${comp_url:-"(not uploaded)"}"; echo; echo "Problem:"; echo " - Expected:"; echo "$q_expected"; echo; echo " - Actual:"; echo "$q_actual"; echo; echo " - Started: $q_started"; echo " - Frequency: $q_freq"; echo; echo "Steps to Reproduce:"; echo "$q_repro"; echo; echo "What I've Tried:"; if [[ -n "$q_attempts" ]]; then echo "$q_attempts"; else echo "N/A"; fi; echo; echo "Error Snippet:"; if [[ -n "$q_error" ]]; then echo "$q_error"; else echo "N/A"; fi; echo; echo "Additional Context:"; if [[ -n "$q_extra" ]]; then echo "$q_extra"; else echo "N/A"; fi; } | pbcopy
    log "Copied to clipboard (pbcopy)."
  elif have xclip; then
    { echo "[${container}] ${q_summary}"; echo; echo "Environment:"; echo " - Image tag: ${image_tag:-unknown}"; echo " - OS/Arch: ${OS}/${ARCH_RAW}"; echo; echo "Links:"; echo " - Docker Logs: ${logs_url:-"(not uploaded)"}"; echo " - Docker Compose: ${comp_url:-"(not uploaded)"}"; echo; echo "Problem:"; echo " - Expected:"; echo "$q_expected"; echo; echo " - Actual:"; echo "$q_actual"; echo; echo " - Started: $q_started"; echo " - Frequency: $q_freq"; echo; echo "Steps to Reproduce:"; echo "$q_repro"; echo; echo "What I've Tried:"; if [[ -n "$q_attempts" ]]; then echo "$q_attempts"; else echo "N/A"; fi; echo; echo "Error Snippet:"; if [[ -n "$q_error" ]]; then echo "$q_error"; else echo "N/A"; fi; echo; echo "Additional Context:"; if [[ -n "$q_extra" ]]; then echo "$q_extra"; else echo "N/A"; fi; } | xclip -selection clipboard
